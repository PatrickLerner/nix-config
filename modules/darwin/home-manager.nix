      ...
      }:
      {
        home = {
          ...
          activation = {
            ...
            # Idempotent: runs 'opencode mcp add' for each server to ensure they are properly
            # registered and configured.
            setupOpencodeMCP = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
            ...
            cloneGithubRepos = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
            ...
            registerOrchestratorRepos = lib.hm.dag.entryAfter [ "cloneInstaffoRepos" ] ''
              JQ="${pkgs.jq}/bin/jq"
            ...
            setupClaudePlugins = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
              JQ="${pkgs.jq}/bin/jq"
            ...
            setFinderSidebar = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
              HOME_DIR="/Users/${user}"
            ...
            registerTaskmdNotes = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
